...
public final class ShopSettings extends InGameUI
{
    ...
    public void OnClickUpperInventory(InventoryClickEvent e)
    {
        ...
        if (e.getSlot() == ENABLE_TOGGLE)
        {
            ...
            data.get().set("Options.enable", !isShopEnable); // สลับค่า enable เป็น true/false
            data.save(); // บันทึกการเปลี่ยนแปลงลงในไฟล์

            DynaShopAPI.openShopSettingGui(player, shopName); // เปิด GUI ใหม่เพื่ออัปเดตปุ่ม
        }
        // 권한
        else if (e.getSlot() == PERMISSION)
        {
            ...
            if (permissionNode == null || permissionNode.isEmpty()) // ตรวจสอบว่าไม่ใช่ null ก่อน
            {
        ...
        } else if (e.getSlot() == CMD_SELL)
        {
            if (e.isShiftClick() && e.isRightClick())
            {
                if (data.get().getConfigurationSection("Options.command.sell") != null)
                {
                    ...
                    data.get().set("Options.command.sell." + idx, null); // เปลี่ยนมาใช้ data.get().set() แทน
                    data.save();
                    DynaShopAPI.openShopSettingGui(player, shopName);
                }
            }
        ...
        } else if (e.getSlot() == CMD_BUY)
        {
            if (e.isShiftClick() && e.isRightClick())
            {
                if (data.get().getConfigurationSection("Options.command.buy") != null)
                {
                    ...
                    int idx = Integer.parseInt(indexList[indexList.length-1].toString());
                    data.get().set("Options.command.buy." + idx, null); // เปลี่ยนมาใช้ data.get().set() แทน
                    data.save();
                    DynaShopAPI.openShopSettingGui(player, shopName);
                }
            }
